...
export default function ChatScreen() {
  ...
  const loadHistoryChat = async (id: string) => {
    setLoading(true);
    try {
      const response = await api.get(`/data/chatshistory/${id}`);
      ...
      if (chat) {
        ...
        // Map messages
        const formattedMessages = chat.messages ? chat.messages.map((m: any, idx: number) => {
          const msg: Message = {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const shareUrl = `https://prepiqo.com/share/${activeHistoryItem._id}`;
      await Share.share({
        message: `Check out this content I generated using Prepiqo: ${shareUrl}`,
        url: shareUrl,
      });
    } catch (err) {
      ...
      const html = `
        <html>
          ...
          <body>
            ...
            ${contentHtml}
            <div class="footer">Generated by Prepiqo - <a href="https://prepiqo.com">prepiqo.com</a></div>
          </body>
        </html>
      `;
              ...
              {msg.type === 'qa' && msg.qa && (
                <View className="mt-2">
                  ...
                  {msg.qa.map((qaItem, idx) => (
                    <View key={idx} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm mb-4">
    ...
    <View style={{ flex: 1, backgroundColor: '#F9FAFB', paddingBottom: insets.bottom }}>
      ...
      >
        ...
        <View className="px-4 pb-6 pt-2 bg-white/90">
          ...
          <View className="flex-row items-center bg-white rounded-2xl border border-gray-100 p-2 shadow-sm">
